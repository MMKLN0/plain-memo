...
当前稳定版本：[PlainMemo 2.1.0](https://github.com/MMKLN0/plain-memo/releases/tag/2.1.0)
...
2. 选择“默认新建位置”。新 memo 会写入这里，该文件夹也会自动加入扫描范围。
3. 按需调整长卡片折叠阈值（最小 6 行）、移动端紧凑布局和时光浮标。

默认不预置任何个人路径或文件夹。未配置扫描目录前，插件不会把 Vault 中的已有文件当作 PlainMemo memo。
